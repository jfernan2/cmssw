...
// bases on the original by Richard Corke, Torbjorn Sjostrand.
...
#ifndef Pythia8_PowhegHooksForBB4L_H
#define Pythia8_PowhegHooksForBB4L_H
...
class PowhegHooksForBB4L : public UserHooks {
...
//--------------------------------------------------------------------------
  ...
  inline bool doVetoMPIStep(int nMPI, const Event &e) {
    ...
    for (int i = e.size() - 1; i > 0; i--) {
      if (e[i].isFinal()) {
        // this may not be process independent but should work both for hvq and bb4l
        // if the particle is a light quark or a gluon
        // and it comes for a light quark or a gluon (not a resonance, not a top)
        // then it is the POWHEG emission in hvq
        // and the POWHEG emission in production in bb4l
        if ( ( abs(e[i].id()) < 6 || e[i].id() == 21 ) && ( abs(e[e[i].mother1()].id()) < 6 || e[e[i].mother1()].id() == 21 ) ) {
                isEmt = true;
                iEmt = i;
        }
        count++;
    ...
    // all other incoming and outgoing partons, with the minimal value taken
      ...
      std::cout<< " We should never have pThardMode == 1" << std::endl;
      exit(0);
    ...
    // taken
      ...
      std::cout<< " We should never have pThardMode == 2" << std::endl;
      exit(0);
...
private:
  int vetoMode, vetoCount, pThardMode, pTemtMode,
...
#endif // end Pythia8_PowhegHooksForBB4L_H
