...
//             2022_02_20  Oscar Gonzalez: Changing the code to use tokens for the ES consumers
//
...
class OglezDTAB7RawToDigi : public edm::stream::EDProducer<> {
...
private:
  ...
  std::vector<L1Phase2MuDTThDigi> primitivesTh_;  ///< Trigger theta primitives to be read 

#ifdef __COMPILATION_EXTENDED_PRIMITIVES__
  std::vector<L1Phase2MuDTExtPhDigi> extPrimitives_;  ///< Extended trigger primitives (Phase2, by N. Trevisani)
  std::vector<L1Phase2MuDTExtThDigi> extPrimitivesTh_; ///< Extended trigger theta primitives
#endif

  edm::ESHandle<DTGeometry> dtGeo_;  ///< To handle the DTGeometry tools.
  edm::ESGetToken<DTGeometry, MuonGeometryRecord> dtGeoToken_;  ///< Token for the ES consumer.

  edm::ESGetToken<DTReadOutMapping, DTReadOutMappingRcd> dtROMapToken_; ///< Token for the ES consumer.
