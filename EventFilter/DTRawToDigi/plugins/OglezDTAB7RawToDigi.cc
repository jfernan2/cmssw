...
//             2021_05_28 Oscar Gonzalez: Using values of factors from L1Trigger/DTTriggerPhase2/interface/constants.h
//             2022_02_20 Oscar Gonzalez: Adapting the code to use the future features (ES consumers, stream producer...)
//                                        and to remove some backward compatibility code (for make the routine cleaner).
//
...
// Constructor of the class
{
  ...
  // New tokens for the ES consumers
  dtGeoToken_ = esConsumes();
  dtROMapToken_ = esConsumes();
  ...
  produces<L1Phase2MuDTExtThContainer>();  // On demand!
#else
...
// Main routine to process in every event.
{
  ...
  L1Phase2MuDTThContainer primContainerTh;
  primContainerTh.setContainer(primitivesTh_);
  auto AB7DTPrimTh_product = std::make_unique<L1Phase2MuDTThContainer>(primContainerTh);
...
//  }
  ...
  e.put(std::move(AB7DTPrimTh_product));

  // Optionally we may save the extended primitives... if requested and compilation allowed!
#ifdef __COMPILATION_EXTENDED_PRIMITIVES__
  if (produceExtendedPrimitives_) {
    ...
    L1Phase2MuDTExtThContainer extPrimThContainer;
    extPrimThContainer.setContainer(extPrimitivesTh_);
    auto AB7DTExtPrimTh_product = std::make_unique<L1Phase2MuDTExtThContainer>(extPrimThContainer);

    e.put(std::move(AB7DTExtPrim_product));
    e.put(std::move(AB7DTExtPrimTh_product));
...
bool OglezDTAB7RawToDigi::fillRawData(edm::Event& e,
  ...
  edm::ESHandle<DTReadOutMapping> mapping = c.getHandle(dtROMapToken_);  // Note we can use token here (assumed to be in "produce", I guess)
...
void OglezDTAB7RawToDigi::process(int DTAB7FED,
  ...
  // (Only done if it is the original RAW data!!!)
  if ( DTAB7InputTag_.label()=="rawDataCollector"
       && ((dataWord>>8)&0x0FFF) != DTAB7FED) {
    if ( debug_ ) edm::LogWarning("oglez_dtab7_unpacker")
...
//        else std::cout<<"OGDT-INFO: Uncorrelated primitive with "<<std::hex<<firstWord<<" "<<dataWord<<std::dec<<" "<<((firstWord>>58)&0x3)<<std::endl;
        ...
        if (((dataWord>>60)&0xF)==0xC)  {  // It is ok! (v6)
          ...
          // The code requires a third word (from v8)
          readLine(&dataWord,1);
          --k;  // We read an extra word!
          // Now we check if it is not ok!
          if (!( ((dataWord>>60)&0xF)==0xD) ) // It is NOT what we were expecting!
            edm::LogWarning("oglez_dtab7_unpacker") << "Wrong number of words to be read for a correlated trigger primitive: no third word? " <<slot_size[slot]<<" "<<k<<std::endl;
...
//OLD  if (payloadVersion_==11) {  // Differences in V11 of the payload... more info
...
//OLD }
//OLD   else {
//OLD     if (thirdWord!=0) {  // In v8 they put a third word with the complete chi2... when it is there.
//OLD       chi2 = ((thirdWord)&0xFFFFFF);   // Bits 0-23 is the full chi2
//OLD     }
//OLD     // We may need to do calculations not needed any more!
//OLD  }
...
//OLD  if (payloadVersion_==11) {  // Now we are just reading the values as indicated above
#ifdef __COMPILATION_EXTENDED_PRIMITIVES__
    ...
    phi_cmssw = (int) round(phiAngle*cmsdt::PHIRES_CONV);  // phiAngle (CMSSW)
    phib_cmssw = (int) round(phiBending*cmsdt::PHIBRES_CONV);  // phiBending (CMSSW)
#endif
...
//OLD   }
//OLD   else if (payloadVersion_==9) { // Better resolution in position information.
//OLD     OglezTransformJMSystem::instance()->getPhiAndPhiBending(slId,dtGeo_,position/16.,  // Using mm as this argument (JM uses mm as metric)
//OLD                                                             jmTanPhi/4096.,quality,&phiAngle,&phiBending);
//OLD
//OLD #ifdef __COMPILATION_EXTENDED_PRIMITIVES__
//OLD     phi_cmssw = (int) round(phiAngle*cmsdt::PHIRES_CONV);  // phiAngle (CMSSW, the only one available)
//OLD     phib_cmssw = (int) round(phiBending*cmsdt::PHIBRES_CONV);  // phiBending (CMSSW, the only one available)
//OLD #endif
//OLD   }
//OLD   else {
//OLD     OglezTransformJMSystem::instance()->getPhiAndPhiBending(slId,dtGeo_,position/4.,  // Using mm as this argument (JM uses mm as metric)
//OLD                                                             jmTanPhi/4096.,quality,&phiAngle,&phiBending);
//OLD #ifdef __COMPILATION_EXTENDED_PRIMITIVES__
//OLD     phi_cmssw = (int) round(phiAngle*cmsdt::PHIRES_CONV);  // phiAngle (CMSSW, the only one available)
//OLD     phib_cmssw = (int) round(phiBending*cmsdt::PHIBRES_CONV);  // phiBending (CMSSW, the only one available)
...
//OLD   }
  ...
  int uind = 0, uphi=0, uphib=0;
  if(superlayer!=2){
  uphi = (int) round(phiAngle*cmsdt::PHIRES_CONV);  // phiAngle
  uphib = (int) round(phiBending*cmsdt::PHIBRES_CONV);  // phiBending
  }
  else{
  uphi = (int) round(phiAngle*cmsdt::ZRES_CONV);
  uphib = (int) round(phiBending*cmsdt::KRES_CONV);
  }
...
// v4 tpindex,  // uind (m_segmentIndex)
  ...
  L1Phase2MuDTThDigi trigprimTh(bx,   // ubx (m_bx)
                              wheel,   // uwh (m_wheel)
                              sector-1,   // usc (m_sector)  USING L1 trigger primitives convention.
                              stationId,      // ust (m_station)
...
// v4 tpindex,  // uind (m_segmentIndex)
  ...
  // Storing the primitive
  primitivesTh_.push_back(trigprimTh);
  }
...
//    }
    ...
    L1Phase2MuDTExtThDigi trigextprimTh(bx,   // ubx (m_bx)
                                      wheel,   // uwh (m_wheel)
                                      sector-1,   // usc (m_sector)  USING L1 trigger primitives convention.
                                      stationId,      // ust (m_station)
    ...
    extPrimitivesTh_.push_back(trigextprimTh);
