...
def shared_hits(muon1, muon2):
    """count how many hits two muons share"""
    ...
    # now count the number of shared hits: 
    # we inlcude also the "non-hits", since we 
    # want to find the best match
    # e.g. a 3-hits TP would share 3 hits with 
    # 3-hits TP but also with a 8-hits TP otherwise
    numShared=0.
    totMuon1=0.
    for ly in range(0,7):
        # if (muon1.pathWireId(ly)>=0): 
...
muoBayesHandle, muoBayesLabel = Handle("L1Phase2MuDTExtPhContainer"), ("dtTriggerPhase2BayesPrimitiveDigis", additional_bayes_label, "L1DTTrigPhase2Prod")
...
for frac in [0.25,0.5,0.75,1.00]:
    ...
    for st in range(1,5):    
        ...
        hMatchingEff2D.append(ROOT.TH2F("hMatchingEff2D_MB%i_%s" %(st,fracname), "", 8, 0.5, 8.5, 8, 0.5, 8.5))
    ...
    for st in range(1,5):
        ...
        outputDict[fracname]['hMatchingEff2D_MB%i'%st] = hMatchingEff2D[st-1]
    ...
    for ev in events:
        ...
        if (dumpToFile):
            ...
            f.write( "        Wh   Se   St  | w1 w2 w3 w4 w5 w6 w7 w8 |  tdc1  tdc2  tdc3  tdc4  tdc5  tdc6  tdc7  tdc8 | lat1 lat2 lat3 lat4 lat5 lat6 lat7 lat8 | Q     phi  phib  bX      Chi2         x tanPsi     t0 \n" )
            f.write( "--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- \n" )
            for muon in muon_bayes:
                f.write( "[Bayes]: Wh%2d Se%2d St%1d | %2d %2d %2d %2d %2d %2d %2d %2d | %5d %5d %5d %5d %5d %5d %5d %5d |  %2d   %2d   %2d   %2d   %2d   %2d   %2d   %2d | Q%1d %6d %5d %3d %9d %9d %6d %6d\n" %(muon.whNum(),muon.scNum(),muon.stNum(),muon.pathWireId(0),muon.pathWireId(1),muon.pathWireId(2),muon.pathWireId(3),muon.pathWireId(4),muon.pathWireId(5),muon.pathWireId(6),muon.pathWireId(7),muon.pathTDC(0),muon.pathTDC(1),muon.pathTDC(2),muon.pathTDC(3),muon.pathTDC(4),muon.pathTDC(5),muon.pathTDC(6),muon.pathTDC(7),muon.pathLat(0),muon.pathLat(1),muon.pathLat(2),muon.pathLat(3),muon.pathLat(4),muon.pathLat(5),muon.pathLat(6),muon.pathLat(7),muon.quality(),muon.phi(),muon.phiBend(),muon.bxNum()-20,muon.chi2(), muon.xLocal(), muon.tanPsi(), muon.t0() ) )
        

        for muon in muon_std:
            if (dumpToFile): 
                f.write( "[Std  ]: Wh%2d Se%2d St%1d | %2d %2d %2d %2d %2d %2d %2d %2d | %5d %5d %5d %5d %5d %5d %5d %5d |  %2d   %2d   %2d   %2d   %2d   %2d   %2d   %2d | Q%1d %6d %5d %3d %9d %9d %6d %6d\n" %(muon.whNum(),muon.scNum(),muon.stNum(),muon.pathWireId(0),muon.pathWireId(1),muon.pathWireId(2),muon.pathWireId(3),muon.pathWireId(4),muon.pathWireId(5),muon.pathWireId(6),muon.pathWireId(7),muon.pathTDC(0),muon.pathTDC(1),muon.pathTDC(2),muon.pathTDC(3),muon.pathTDC(4),muon.pathTDC(5),muon.pathTDC(6),muon.pathTDC(7),muon.pathLat(0),muon.pathLat(1),muon.pathLat(2),muon.pathLat(3),muon.pathLat(4),muon.pathLat(5),muon.pathLat(6),muon.pathLat(7),muon.quality(),muon.phi(),muon.phiBend(),muon.bxNum()-20,muon.chi2(), muon.xLocal(), muon.tanPsi(), muon.t0() ) )
            ...
            for muon2 in muon_bayes: 
                ...
                # # consider as the matched muon the one with quality closer to muon.quality()
                # # if two muons have the same quality "distance" from muon, keep the highest quality one
                # # use this only in case of 2D efficiency plot
                # if IsMatched(muon,muon2,frac) == True and muon_matched != 0:   
                #     if ( (abs(muon.quality() - muon2.quality()) <= abs(muon.quality() - muon_matched.quality()))
            ...
            if best_matched != 0:
                hMatchingEff2D[st].Fill(muon.quality(), best_matched.quality())

            hMatchingEff_MB[5*st + wh].Fill(matched, muon.quality())
            

    for st in range(0,4):
        for AM_quality in range(1,9):  
            print("Station: {}".format(st))
            total_muons = 0 
            for Bayes_quality in range(1,9):
                total_muons += hMatchingEff2D[st].GetBinContent(AM_quality, Bayes_quality)
            for Bayes_quality in range(1,9):
                if total_muons != 0:
                    hMatchingEff2D[st].SetBinContent(AM_quality, Bayes_quality, hMatchingEff2D[st].GetBinContent(AM_quality, Bayes_quality) / total_muons)
        hMatchingEff2D[st].GetZaxis().SetRangeUser(0.0, 1.0)
...
import pickle 
save_name = "GroupingComparison_StdToBayes{}.pickle".format(additional_bayes_label)
