...
for st in ["MB1","MB2","MB3","MB4"]:
    for wh in ["Wh-2", "Wh-1", "Wh0", "Wh1", "Wh2"]:
        ...
        if wh == "Wh-2":
            ...
            # 2D
            plots.append("hMatchingEff2D_%s" %(st))
            titles["hMatchingEff2D_%s" %(st)] = " ; AM quality ; Bayes quality"  


outpath = "/afs/cern.ch/user/n/ntrevisa/work/DT/CMSSW_11_2_0_pre2/src/Groupings/"
if additional_bayes_label != "":
    outpath = outpath + additional_bayes_label + "/"
...
output_file_name = "GroupingComparison_StdToBayes{}.root".format(additional_bayes_label)
...
file_name = "GroupingComparison_StdToBayes{}.pickle".format(additional_bayes_label)
...
for plot in plots:
    ...
    disp2.SetTextSize(0.025)
    for s in ss: 
        if not (drawn): 
            if ('2D' in b[s][plot].GetName()):
                b[s][plot].Draw("colz")
            else:
                b[s][plot].Draw()
                drawn=True
        ...
        if ('2D' in b[s][plot].GetName()):
            ...
            disp.DrawLatexNDC(0.35,  0.26, "Fraction of AM muons of a given quality")
            disp2.DrawLatexNDC(0.35, 0.22, "matched to a Bayes muon of a given quality")
            disp.Draw("same")
            disp2.Draw("same")
            ROOT.SaveCanvas(canvas, outpath + plot + s)
        elif (b[s][plot].InheritsFrom("TH1")):
