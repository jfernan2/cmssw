...
void MuonPathSLFitter::analyze(MuonPathPtr &inMPath,
  ...
  for (auto &lat_comb : lat_combs) {
    ...
    coeffs_t coeffs;
    if (sl == 0) {
      coeffs = RomDataConvert(
          lut_sl1[rom_addr], COEFF_WIDTH_SL_T0, COEFF_WIDTH_SL_POSITION, COEFF_WIDTH_SL_SLOPE, 2 * sl, 2 * sl + 3);
    } else {
      coeffs = RomDataConvert(
          lut_sl3[rom_addr], COEFF_WIDTH_SL_T0, COEFF_WIDTH_SL_POSITION, COEFF_WIDTH_SL_SLOPE, 2 * sl, 2 * sl + 3);
    }
