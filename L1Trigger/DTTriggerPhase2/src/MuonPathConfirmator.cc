...
// ============================================================================
MuonPathConfirmator::MuonPathConfirmator(const ParameterSet &pset,
                               ...
                               std::vector<cmsdt::metaPrimitive> &outMetaPrimitives)
{
  ...
  int position_prec = (int) (mp.x * std::pow(2, PARTIALS_PRECISSION));
  int slope_prec = (int) (mp.tanPhi * std::pow(2, PARTIALS_PRECISSION));

  int slope_x_halfchamb = int(slope_prec) * SEMICHAMBER_H >> SEMICHAMBER_RES_SHR;
  int slope_x_3semicells = int(slope_prec * 3) >> LYRANDAHALF_RES_SHR;
  int slope_x_1semicell = int(slope_prec * 1) >> LYRANDAHALF_RES_SHR;
  ...
  for (const auto &dtLayerId_It : *dtdigis) {
    ...
    if (isSL1 != hitFromSL1) { // checking hits have the opposite SL than the TP
      for (auto digiIt = (dtLayerId_It.second).first; digiIt != (dtLayerId_It.second).second; ++digiIt) {
        ...
        double position_in_layer = position_prec + (-1 + 2 * (int) isSL1) * slope_x_halfchamb;
        if (ly == 0)
        ...
        position_in_layer /= std::pow(2, PARTIALS_PRECISSION);
        ...
        } else if ((std::abs(position_in_layer - hit_position) >= minx) && (std::abs(position_in_layer - hit_position) < min2x)) {
